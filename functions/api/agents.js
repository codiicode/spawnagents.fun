export async function onRequest(context) {
  ...
  try {
    ...
    let query, params;
    if (status) {
      query = "SELECT id, name, parent_id, generation, owner_wallet, agent_wallet, dna, meta, status, total_pnl, total_trades, born_at, last_trade_at FROM agents WHERE status = ? ORDER BY born_at DESC LIMIT 100";
      params = [status];
    } else {
      query = "SELECT id, name, parent_id, generation, owner_wallet, agent_wallet, dna, meta, status, total_pnl, total_trades, born_at, last_trade_at FROM agents ORDER BY born_at DESC LIMIT 100";
      params = [];
    }
